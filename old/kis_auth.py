...
config_root = '/Users/macm1/git/korea-invest'  # 토큰 파일이 저장될 폴더, 제3자가 찾지 어렵도록 경로 설정하시기 바랍니다.
# config_root = '/Users/P170355/git/vscode-ws/korea-invest'  # 토큰 파일이 저장될 폴더, 제3자가 찾지 어렵도록 경로 설정하시기 바랍니다.
...
local_temp = config_root + '/local-temp/kis-token'  # 토큰 파일이 저장될 폴더, 제3자가 찾지 어렵도록 경로 설정하시기 바랍니다. 
token_tmp = local_temp + '/KIS' + datetime.today().strftime("%Y%m%d")  # 토큰 로컬저장시 파일명 년월일시분초

# 접근토큰 관리하는 파일 존재여부 체크, 없으면 생성
if os.path.exists(token_tmp) == False:
    if os.path.exists(local_temp) == False:
        os.makedirs(local_temp)
    f = open(token_tmp, "w+")

# 앱키, 앱시크리트, 토큰, 계좌번호 등 저장관리, 자신만의 경로와 파일명으로 설정하시기 바랍니다.
# pip install PyYAML (패키지설치)
with open(config_root + '/config/kis_devlp.yaml', encoding='UTF-8') as f:
...
print("샘플을 쓰레기가 만들었나... 동작을 안해... 어이가 없네...")
print(f"config_root={config_root}", "환경정보", _cfg)
